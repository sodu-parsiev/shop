<section id="catalog" x-data="catalogFilter" class="border-b border-brand-black/10 bg-brand-black py-16 text-white lg:py-28">
    <div class="mx-auto max-w-[1324px] px-5 sm:px-8 lg:px-0">
        ...
        <div class="mt-8 grid grid-cols-1 border-y border-white/20 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="border-b border-white/20 p-4 lg:border-r lg:border-b-0">
                ...
                <select
                    x-model="$store.orderBuilder.preferredDensity"
                    class="mt-2 w-full border-0 bg-transparent px-0 py-1 text-sm font-bold text-white ring-0 focus:ring-0"
                >
                    <option class="text-brand-black">{{ $homeContent->get('catalog.filter_density_default') }}</option>
                    @foreach ($homeContent->get('catalog.filter_density_options', []) as $option)
                        <option class="text-brand-black">{{ $option['value'] }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="border-b border-white/20 p-4 sm:border-r sm:border-b-0">
                ...
                <select
                    x-model="$store.orderBuilder.preferredSize"
                    class="mt-2 w-full border-0 bg-transparent px-0 py-1 text-sm font-bold text-white ring-0 focus:ring-0"
                >
                    <option class="text-brand-black">{{ $homeContent->get('catalog.filter_size_grid_label') }}</option>
                    @foreach ($homeContent->get('catalog.filter_size_options', []) as $option)
                        <option class="text-brand-black">{{ $option['value'] }}</option>
                    @endforeach
                </select>
            </div>
